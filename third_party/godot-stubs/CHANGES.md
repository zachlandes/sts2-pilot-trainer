...
| `Core.cs` | `Node.GetIndex` and `Node.MoveChild` added, beside the private child list they both need. Two of this mod's surfaces are inserted into the game's own containers at a particular place - the recorder's row under the overlay's MODDED label, the settings row after the game's own modding row - and where they land is exactly what went wrong when it was guessed rather than asserted. The real GodotSharp has carried both all along; the gap was only ever in this stub. |
| `UI.cs`, `Sts2PilotTrainerAdditions.cs` | The four font members of `Control` - `AddThemeFontOverride`, `AddThemeFontSizeOverride`, `HasThemeFontOverride`, `GetThemeFont` and `GetThemeFontSize` - moved out of the vendored snapshot and now back two dictionaries in this project's own file, the way `AddThemeColorOverride` already backed `ThemeColorOverride`. Upstream's versions forgot what was set: the override answered false, the size answered 16 whatever had been written, and the font answered a fresh instance. Every Runmobile surface now takes its font and size from a native label rather than from a number of its own, and that reading is exactly the distinction upstream's stub could not make - a control wearing the game's type from one wearing Godot's. The real GodotSharp has carried all five all along; the gap was only ever in this stub. |
