...
The projection carries nothing of a fight outside a live one - `CanonicalStateProjection.ProjectCombat` emits `combat.in_progress` and `combat.outcome` and nothing else once the combat manager says no fight is running, with the outcome read off the room the run stands in rather than off the finished `PlayerCombatState` the engine keeps until the next fight - so a reading taken at a shop, a rest site, an event or a loot screen after a won fight and the reading of a run the game restored there from its own save are the same state, digest for digest.
...
`RunJournal.Schema` is v5: v4 made the promise that every save the game asked for is on the file, and v5 that no reading carries a finished fight, so every complete digest on the file is one this projection reproduces; a journal declaring any older schema is refused on resume, as a v1 always was, because a recording continued from one would be two projections in one file, and the run is simply not continued as a recording.
