...
4. Play a run. `[Runmobile] recording this run as native-<seed>-<date>-<time>` says it attached.
   ...
   Two plays either side of an `EndTurn` do not serve `reorder-plays` either: the hand is discarded and redrawn at the end of a turn, and the control re-indexes both plays against one shared hand.
