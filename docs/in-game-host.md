...
Shell ownership is paid for at the other end: `InstallShellPatches` treats a patch class Harmony cannot resolve as a broken build and lets it throw out of `Start`, so a game update that renames a `CardSelectCmd` entry point `CardPrompts` patches, such as `CardSelectCmd.FromHand`, or `NCardRewardSelectionScreen.OptionSelected` takes the whole mod down, where the same rename behind a module's own patches would disable only that module.
...
A resuming decision that is not the one the observer opened is marked incomplete by the observer itself, because a choice it never saw begin is one it cannot account for.
...
Waiting uses only what the section below records as working here, on the `SettleClock` the attach site handed `PlayerFightObserver.Start`: the scene tree's timer in the retail client, and headlessly the arbiter's drain, which [headless-fidelity.md](headless-fidelity.md#recording-a-run-headless-and-the-two-seams-it-needs) owns.
...
Leaving through the game's menu - Save and Quit, Abandon Run - records nothing and shows nothing: the teardown patch cannot tell those apart from Jump to the end, and only the last asked to see a result, so the "left before it ended" notice is queued by `RecordedFightRun.LeaveTheFightNow` alone, before it tears the run down, and shown over the main menu once the return finishes.
The first build queued that notice from the teardown as well, and a player who saved and quit a replayed fight came back to the menu to a popup about a comparison they had not asked for; `SavingAndQuittingATrainerFightQueuesNoResultAndLowersTheBarrier` and `JumpingToTheEndIsTheOneRouteToTheLeftNotice` in `ProfileWriteBarrierTests` hold the two routes apart.
...
`RunRecorder` in `Sts2PilotTrainer.Mod` attaches when a run starts, watches every decision the player makes, and writes a v8 native manifest under `user://Runmobile/recordings/` when the run ends.
...
The one question the two sinks answer differently is an action whose argument the observer could not resolve, which is why `IFightSampleSink` asks it rather than the observer deciding: a history missing an argument the format requires is a run nobody can replay, so the recorder stops the recording at that action through `MarkUnmapped` (the in-fight stop under the integrity paragraph below), while a fight being compared never reads that argument and the capture keeps the step.
...
`RunRecorder` decides which: `Refuse` is a hole in the watch - a decision that went by unread, an engine that never settled, a resume the journal cannot place - and calls `MarkBroken`; `StopAt` is a decision the recorder saw and could not name - a reward this format has no verb for, a shop entry on no shelf it knows, an answer that is not among what the screen offered - and calls `MarkUnmapped` with the member or screen it was seen at, the game's own values as strings, and why on the stop's evidence note.
The distinction is the recording's own: a stop leaves continuity where it was, so the validator names the decision the recorder met, and only a hole gets the sentence about a watch that stopped and started again.
A stop met at a member outside a fight is queued through the pump behind the decisions still settling, because a stop stands at the ordinal after the last decision recorded; one met as a screen's answer is held beside that screen's answers and stops the decision that opened the screen, at that decision's ordinal and with its before-reading, since a decision recorded without its answer is one a replay makes differently.
...
A stop met inside a fight is the observer's play whose argument it could not resolve, told to the recorder's sink through `IFightSampleSink.BeginStepWithUnresolvedArgument`, and it names the game's own action - the `GameAction` type the observer met, `PlayCardAction` - with the format verb as its discriminator, because the verb is this recorder's translation and names nothing a later build can read back to the game; `RunRecorder.StopAtFightStep` is the one writer of a stop inside a fight, whichever seam met the decision.
A decision that arrives by no member the recorder patches is met at one of three seams and stopped there by name rather than dropped, because a stranger the member patches never see is the class the stop exists for - a game action a mod added or a game update introduced, a choice synced by a prompt nobody patched.
`RunRecorder.ActionRequested` is a prefix on `ActionQueueSynchronizer.RequestEnqueue`, the local origin of every game action in a singleplayer game, and classifies each action instance once against `NetActionClaims` in the engine: a claimed type is recorded by its own patch or the fight observer and takes its usual path, the engine's own `ReadyToBeginEnemyTurnAction` is excused, the console's marks the run non-standard the way the console patch does, and anything else stops the recording at the `net_action` seam naming its type; an action the synchronizer defers past the enemy turn re-enters that member when the player's turn begins, which is why an instance is classified once.
`NetActionClaims.Verify` holds the table to `DecisionSurface.NetActions`, the walk off the assembly, in both directions, so a twelfth game action fails a test before a recorder meets it.
`PlayerFightObserver.BeforeAction` has the same default at the executor for a player-driven action that is none of the five a fight is made of, through the `unmapped` callback the recorder hands it at attach; the recorded-fight journey passes none and keeps its silence.
That callback carries what `IFightSampleSink.BeginStep` carries for a decision - the state sampled before the stranger and whether the step still open had finished - and `RunRecorder.StopAtFightAction` closes that step through the same `CloseStrandedFightStep` every decision closes its predecessor through before it stops through `StopAtFightStep`, so a stranger a played card enqueued, met after the play executed and before its after-sample, closes the play on the stranger's before-sample and stops at the ordinal after it rather than taking the play's ordinal and dropping the play.
Inside a watched fight `ActionRequested` leaves a stranger the observer `Watches` - the player's own and player-driven - to the observer for that reason, because a stop written at the request stands at the open step's ordinal; the observer is the one reader of which actions it meets.
`ChoiceSynced` stops at the `player_choice` seam on an answer synced through `PlayerChoiceSynchronizer.SyncLocalChoice` that nothing here claims: not a bundle or relic screen it holds open, not a card prompt `CardPrompts` holds open or a card reward's screen `CardScreensUp` counts, not a prompt the game's own selector answers in both hosts, and not a choice made inside a decision the recorder has announced and not yet settled - which is how a rest site's Mend, asking which player to heal inside `ChooseRestSiteOption`, and a card reward's pick inside the reward it opened are read as the decisions they belong to.
`RecorderSeamDefaultTests` drives a game action defined in the test through both seams headlessly, holds the engine's readiness action and a game-owned selector's answer to no stop, and stops on an unclaimed synced choice by its kind.
...
Every decision line carries `before` and `before_digest` beside the settled `state` and `digest`: the reading taken in the prefix of the member the decision went through, which is the state the player made it from and the instant a comparison at verification asks about.
...
The current journal schema and why older schemas refuse on resume are recorded with the continuity contract below.
...
`CardPrompts` patches every `CardSelectCmd` entry point that reaches a screen - ten of them, and not the five that forward to one of the ten, which patched as well would announce one prompt twice - and never a screen.
The five are `CardPrompts.Forwarders`, each with the entry point it forwards to, and `RunRecorderTests` holds that table to the game's IL: every public choice entry point is patched or in it, each forwarder's own body calls the entry point it names and opens no prompt of its own, and nothing outside `CardSelectCmd` creates or shows a card-selection screen unless it is excused by name with its reason, so a game update that has a forwarder open a prompt directly in its own body, or opens one from a relic directly, fails naming itself rather than recording nothing.
The reading is one forwarder level deep by design: a prompt a forwarder reaches through a private helper of `CardSelectCmd` is attributed to the funnel and is not caught here.
...
The count `CardScreensUp` keeps is taken only from the moment a prompt is offered, in the one try/finally around the entry point's task, and never from the call: a hook's prompt whose action the fight ended before it ran is asked and never offered, and its task never settles - `GenericHookGameAction` completes its execution-started source only when it executes - so a count taken at the call would never come back, and every settle after it, in that run and in every run of the same session, would wait without a budget while the recording ended broken with the rest of the run stranded.
That prompt is dropped as the open one when the fight ends, at the head of both of the game's paths there, the win's `EndCombatInternal` and the loss's `ProcessPendingLoss`, and again when the run is torn down, so it is not the conflict the next prompt meets; a prompt still genuinely open when another is asked is still a conflict and stops the recording.
A prompt that was offered and counted is given back no later than the run's teardown as well, because the hand's own `_ExitTree` completes its prompt with an empty answer rather than cancelling it - the grid and choose-a-card screens cancel theirs - so `FromHand` and `FromHandForUpgrade` go on to wait for an action `RunManager.CleanUp`'s queue reset will never resume, and a count held to that task would be held for the rest of the process: the count is the current run's, `CardScreensUp.RunTornDown` starts a fresh one at zero, and a prompt takes from and gives back to the run it was counted in, so one of a torn-down run that settles later, or never, can neither hold the next run's count up nor take it below zero.
What the recorder does with an answer is `RunRecorder.HoldCardPromptAnswers` and nothing else: a prompt that asked for exactly N and was answered with N picks is written as `SelectCardFromScreen` records after the decision that opened it; a prompt that asked for a range - `MinSelect` below `MaxSelect`, which every choose-a-card screen is - is written as its picks and then one `ConfirmCardScreen` with their count, zero for a prompt declined, which is how an "up to N" answered with fewer and a skipped choose-a-card screen are stated; and a count outside what the prompt asked for, a prompt that settled before the engine paused for it, or two prompts open at once each stop the recording `unmapped` at that decision, naming what was met, because a recording that wrote less would refuse at replay in front of a player.
`ManifestCardSelector` reads the count back by the same rule, so which prompts take a confirmation is decided by the prompt's own range on both sides; [docs/headless-fidelity.md](headless-fidelity.md) owns the replay's half.
...
Nothing weaker is asked.
The projection carries nothing of a fight outside a live one - `CanonicalStateProjection.ProjectCombat` emits `combat.in_progress` and `combat.outcome` and nothing else into the digest once the combat manager says no fight is running, with the outcome read off the room the run stands in rather than off the finished `PlayerCombatState` the engine keeps until the next fight - so a reading taken at a shop, a rest site, an event or a loot screen after a won fight and the reading of a run the game restored there from its own save are the same state, digest for digest.
The one reading of a finished fight it takes, which side's turn the fight ended in, goes through `CanonicalState.Builder.AddOutsideTheDigest` as `combat.ended_on_side`: sampled into a trace for the comparison in [comparison-direction.md](comparison-direction.md), left out of the digest and out of every comparison that holds two hosts to one state (`ReplayTrace.HeldToTheDigest`), because the restored run has no such reading to agree on.
Through format 6 the projection emitted that finished fight into every reading until the next one while `SerializableRun` carries no combat, and the resume compared the two readings with every `combat.` field taken away where either carried a finished fight, through a second digest written beside every decision; that filter, that digest and the residue it existed for are gone together, and `FinishedFightResidue` in `Sts2PilotTrainer.Replay` is what remains of them: the reading of a format-6 file, and which of its boundaries this build cannot reproduce.
The step that ends a fight while the run goes on is read once the engine has settled, through the same pump every out-of-fight decision goes through, rather than at the moment the observer closes it: the killing action finishes before the engine ends the combat and takes the fight-won save, and headlessly the rewards are rolled before the engine drains, so a reading taken at the observer's close names a state the headless replay never holds.
A fight the run was lost in is read where the observer closes it, because nothing rolls after a loss and `Finish` follows; and `Finish` reads a step still waiting on the pump before it counts what was left unread, so a won run's killing blow is in the history it finishes.
`Finish` also takes the one decision left pending where the engine is quiet, through the same `EngineIsQuiet` the settle polls: the game wins a run from the Architect's PROCEED and `RunManager.OnEnded` runs inside that press, so under Instant fast mode the decision the run was won on is still queued when the recording is finished, and a `Finish` that counted it as unread ended every won recording `broken`; a decision still in flight, or one of several, is left to that count, because no state this reading could take is honestly its after-state.
After those decisions and their saves have settled, `Finish` reads Harmony's registry again and appends that captured patch roster to the journal before it writes the manifest.
The start reading stays in the journal header, so a resumed recording compares the final registry with the one the first session actually saw rather than with a new start reading.
That line is also what marks the journal as ended: `RunCapture.Resume` refuses a journal that already carries it, because the run it describes finished and a Continue after that is a new run the recorder did not witness.
The headless replay reads the same instant through `RunEnding`, which `docs/headless-fidelity.md` owns; `AWonRunFinishesContinuousWithTheDecisionItWasWonOnAndReplays` holds the two readings equal, and `demo/RUNMOBILE-WON-RUN.md` is the headless proof.
The retail client rolls the rewards later still, on its own clock in `NCombatUi.ShowRewards` after the settle, and the game's restore of the fight-won save rolls them again as it re-enters the room, so the run comes back at the state the *next* decision began from - the claim, the skip - and never at the killing play's settled reading; `RunCapture.Resume` matches that one entry through its successor's `before_digest`, and through the sample where the player quit before deciding anything on the loot screen, and no other entry gets that latitude (`RunCapture.MatchesTheRestoredLootScreen`).
A recorder that compared the killing play's own reading found this in the client and nowhere else: the first retail run on this change resumed broken on the loot screen with every sampled field agreeing.
Which decision a save belongs to is a ticket rather than a flag: every before-reading takes one, open from the prefix that read it until the pump has dealt with the decision, and a save asked while any is open is placed on the latest.
The rule that closed every older ticket at a commit placed the shop arrival's save on the loot screen's skip, because the client declines the skip from inside the map move that leaves the room, so the move's ticket is older than the skip's and still executing when the skip commits; only the committed ticket closes now, and the ticket is the one thing that places a save.
A save asked on a ticket whose decision never reaches the pump - a prefix read, and the game member threw - is never placed, and the recorder lets it go when the recording finishes with one log line counting them (`RunRecorder.DropTheSavesNeverPlaced`).
A fallback that placed such a save on whichever decision committed later was removed in review, because a save point naming a decision the save does not hold makes a reload to that decision read as the game's own return, and a recording continuous by that reading is shareable; a save never placed only costs the honest Continue after it its continuity, which is the side to err on.
`RecorderContinueTests` opens a map move's prefix without its postfix, asks a save on it and commits three decisions past it, and holds the save points to the game's own.
Where a resume refuses, the recorder logs the fields the run came back different in beside the refusal, bounded to eight fields and a line's worth of value each (`RunRecorder.DescribeResumeDifferences`), because every diagnosis of a broken Continue before it had to be rebuilt from the journal.
[demo/RECORDER-CONTINUE-RESIDUE.md](../demo/RECORDER-CONTINUE-RESIDUE.md) has the loot screen, the map and a treasure arrival continued in the retail client on this rule, with the log lines each Continue produced.
...
The one unequal state that remains continuous is the game's own return to its latest save.
The game saves the run at five moments and nowhere else - as the run begins, inside every map-point arrival before the room type is rolled, when a fight is won, when an ancient event finishes, and again with the reload count on Continue - and Continue restores the latest of them, so what was done after it is undone and re-offered: the fight rolled back to its entry, the loot back on the screen, the shelf restocked, the rest site untaken, the event page unturned.
That is the game working as designed and not a hole in the watch.
Whether those are still the moments on a build is read from the assembly rather than trusted: `scripts/save-points.txt` records every `SaveManager.SaveRun` overload, the members that call each and the `AncientEventModel` subclasses, and `RunRecorderTests.TheGamesSaveContractIsTheRecordedOne` holds it, so a build that saves somewhere new, or adds an ancient event, fails there before a Continue from it reads as broken; `AGENTS.md` owns when the record is regenerated.
An act change leaves that record untouched - a new or alternative act saves at the same call sites and adds no ancient event - so `scripts/act-topology.txt` records the acts the database ships at each index, the default and the epochs that unlock each, and the unshipped `ActModel` subclasses, held the same way by `RunRecorderTests.TheGamesActTopologyIsTheRecordedOne`, so a build whose acts moved fails there before the fixtures' progression is stale.
The recorder observes the game's own `SaveManager.SaveRun` through one postfix (`RunRecorder.RunSaved`), places each save on the decision it holds the state after - the decision in flight from its prefix to the pump writing it, or the last one recorded where nothing is - and writes it to the journal as a `save_point` line only once the game's own save task has completed, because Continue restores what is on the disk and a save that never landed would read the honest Continue after it as a reload of an older save.
The run-start save and the Continue's own come before the recorder attaches and are never written; the first is what a rollback to the opening reading returns to.
`RunCapture.Resume` then accepts as the game's own the return to the decision the latest save point names, or to the opening reading where none has landed, with decisions observed after it, whatever room the save was taken in; those decisions remain in an append-only rollback record and in `source.native.discarded`, while the replayable history resumes at the save's decision.
`RunJournal.Schema` is v6: v4 made the promise that every save the game asked for is on the file, v5 that no digest hashes a finished fight, and v6 that a finished journal carries the patch roster read at run end.
A journal declaring any older schema is refused on resume because a recording continued under a different projection or without the current receipts would mix two recording contracts in one file.
That refusal is `UnreadableJournalSchemaException`, the one refusal out of `RunJournal.Parse` that says nothing about the recording; `./scripts/arbiter parity` counts a journal refused that way as holding nothing, where every other `Parse` refusal is a recorder defect or corruption that fails its figure.
`RunJournal.Parse` holds a rollback line claiming to be the game's own to the latest save point on the file at that point, drops the save points on the branch a rollback discards the way it drops the bookmarks, and refuses a save point naming a decision the history has not got, so a journal cannot be edited into a continuous one.
The manifest carries `source.native.save_points` as captured facts, present and empty where the game saved only at run start, and `ManifestValidator` holds every branch that is not a reload's to one of them or to the opening reading; a recording from before the list existed is held to the fight-only shape instead.
That list is the continued history's alone, so a reload that rewinds behind an earlier own rollback takes the save that rollback returned to off it; the branch keeps that save as its own `save_point`, captured at the decision it returned to, and the validator holds the branch to it - the same decision, listed or taken off the list by exactly a later reload's branch - rather than to a list the reload has since shortened, and reads the branch's boundary decision from the branch that reload discarded, where it now is.
A branch written before it carried one is held to the list alone, and a reload's branch carries none.
Without that, the first Continue-then-older-reload left a rewound recording the validator refused, which is a run the player could not play from; `RunCaptureTests` and `RecorderContinueTests` each hold the sequence.
For publication, `gate` replays every discarded branch from the history as that branch saw it through the decision it left from and holds the branch's state there and at its end against the engine's, exactly - a branch played after a Continue was recorded on a restored run, which carries none of the last fight or a fresh combat state in its place where the engine carries the fight as it was fought, and the projection reads the two the same - and, where the save was a floor arrival the continued history still holds, holds the branch's digest to the verified boundary there.
A run continued outside a fight after a won one is therefore continuous, validates, is the player's to play from and reproduces through `gate` at every checkpoint, every declared boundary and every branch: the retail run in [demo/RECORDER-SAVE-POINTS.md](../demo/RECORDER-SAVE-POINTS.md) failed reproduction at its first non-fight arrival after the Continue while the projection carried the finished fight, and [demo/RECORDER-SAVE-AND-QUIT.md](../demo/RECORDER-SAVE-AND-QUIT.md) is three Save and Quits in one retail run passing every engine condition once it did not, short of only the `rejection` condition's standing demand that a run offer every negative control something to damage.
The `omit-play` control moves a native recording's save points, rollbacks and stop back past the play it removes, the way it moves checkpoints and boundaries, because a control the validator refuses at ingestion never reaches the engine it exists to test; the first Save-and-Quit recording gated in the client failed its rejection condition on exactly that.
That change moved every committed boundary digest at a non-fight arrival after a fight, and is manifest format 7; `FinishedFightResidue` owns what a format-6 file carried and format 7 does not, `ManifestJson` reads a format-6 file with its finished-fight expectations taken away, `gate` names a format-6 boundary this build cannot reproduce, and `migrate-manifest --derive-boundaries` re-derives exactly those from a verified replay.
A live state that matches an earlier save than the latest is a reload that rewound the run, the `rewound` case in the next paragraph.
...
Both cost the recording any chance of being shared; only one of them stops the recording, and until this was separated every refusal did both.
A resume whose live digest matches an earlier decision in the recorder's own history than the game's latest save is a reload that rewound the run behind what had been recorded - the player continued from an older backup or a cloud copy, which the game lets them do and which the mod does not prevent.
...
The first build of this kept the abandoned decisions in the history and went on appending after them, which the browser offered and the entry aborted on; `RewoundRunPlaybackTests` drives a rewound copy of the committed recording into `enter-fight` and `gate` so that cannot come back.
A reload that rewinds behind an earlier rollback's return point remakes the decisions that earlier branch was played from: they become the reload's own branch, and the continued history holds what was played in their place.
`DiscardedBranchHistory` in `Sts2PilotTrainer.Replay` is the one reader of the history as a branch saw it - a decision is the nearest later branch's whose rollback rewound behind it, and the continued history's where none did - and both `Arbiter.RunDiscardedBranch` and `ManifestValidator` read through it, so a branch replays from the decisions it was played after and is held to a verified boundary on the continued history only where that history still holds the decision.
Replayed from the remade history instead, the first branch of a rollback-then-older-reload ran its shop purchase in a room that was never a shop; `RecorderContinueTests` drives that recording headlessly through the whole history and every branch.
A resume whose digest matches nothing the recording ever saw is the other thing and stops the watch, because nothing establishes what the run even is from that point, and a hole outranks a rewind: a reload placed after it leaves the recording `broken`.
...
What a mod actually patched is a separate reading, taken from Harmony rather than from anybody's manifest and captured at run start and run end as `environment.mods.patch_roster`; [environment-identity.md](environment-identity.md) owns it and the three refusals over it.
...
The settle then waits for that work as it waits for any decision's, with one exception `RunRecorder.HandedToThePlayerDuring` owns and four decisions' work opts into, through `PendingDecision.SettlesOnceHandedToThePlayer` - the event option's, the purchase's, the rest option's and the reward claim's: work that has handed the run to the player - a rewards set on offer, read where `RewardsSetSynchronizer.BeginRewardsSet` begins one, or the Crystal Sphere's screen up, read where `CrystalSphereMinigame.PlayMinigame` puts it up, either begun while this decision was the one executing - has settled, because what answers it is the player's next decisions.
Asked of the decision and not of the run: a set on offer beside a reward being claimed off it was begun under an earlier ticket and is not this claim's wait, so the claim's own work - the gold flying, the card landing - is waited for as before.
A claim, a purchase or a rest whose own work begins a set and awaits it - Orrery or the Cauldron bought from a shop, a heal under Dream Catcher or the Tiny Mailbox, a relic claimed off Neow's Bones that opens rewards of its own - is the case the four opt in for: that task finishes only once the set is answered, by the decisions recorded after it, so a recorder that waited for it ran the budget out and refused every such purchase, rest and claim in normal play, which the Stage 2 coverage rows for those relics found.
The driver replays those decisions the same way, through `RunDriver.SettleOrHandOver`: the queue drained, and a task still open with a set on offer that the work itself began handed over rather than awaited on the one thread the answering decisions arrive on - a set already on offer before the work started, the loot screen's beside a claim off it, is not the work's to wait on and such a task is awaited as before; each host reads one state, the set on offer and the engine quiet, and `GeneratedCoverageTests` holds the recording of each to a fresh replay through `TraceParity`.
A refusal the handed-over task ended in is read at the end of every action as well as at the start of the next, because the decision that answers the set is often the recording's last - a relic bought and its rewards taken, then the run given up - and a refusal read only at a next action that never comes left the replay verified; `HandedOverWorkTests` holds both the end-of-step read and the already-on-offer case with a stand-in task.
A card prompt, the bundle screen and the relic screen are waits of a third kind and need no entry there: the settle stands down for each of them on its own count - `CardScreensUp` for a prompt, `BundleScreen.Open` and `RelicScreen.Open` for the two screens stood in for at the prompt itself - and the option's task finishes once it is answered; Neow's Scroll Boxes is the case, a blessing whose task awaits the bundle screen for as long as the player deliberates.
...
The headless engine has no hit animation, which is why no headless capture ever saw this; `RecorderTimingTests` stands one in with a task the test completes after frames have gone by, on `PumpedSettleClock.Tick`, holds the decision's after-reading to the state the rewards are on offer in, and holds the recording to a fresh replay through `TraceParity` on a seed whose first act rolls Brain Leech behind its first fight; on the old reading the oracle names hidden state at the option with every sampled field agreeing, which is the store run's own failure.
...
That proof is headless only.
In the retail client `LordsParasol.AfterRoomEntered` starts its purchases without awaiting them, and they wait on `Cmd.Wait` scene-tree timers that only a headless host or Instant fast mode skips; `EngineIsQuiet` reads the executor and the queue, which are idle while a timer runs, so the map move into the shop can settle and be written before any purchase or the relic's forced removal prompt has happened.
The move's after-reading is then taken before the purchases the replay's move makes, and the prompt's answer is held under the next decision rather than the move in; both are a known limitation owed to the retail-timing follow-up, not a fixed defect, and the recorder's settle rule is unchanged here.
...
Every one of those refusals is `RetailSoak.RefusalToRun`'s, and each is written to `soak-done` as zero runs with the sentence in its `refusal` field before the client quits the way a finished night's does, because a refusal that only wrote to the log left the client at the main menu and the script waiting out the night's whole budget to report nothing; the script exits 3 on it, naming the sentence.
A settings file the client could not read at all is read as no plan, and is the one refusal that has to tell a soak client from a player's before it writes: a player's client with such a file keeps its menu and records nothing, and the soak's is told apart by `RetailSoak.IsASoakClientWithoutAPlan` on the one fact no player's client has - a headless display server - since a windowless game is not playable and the soak script launches under nothing else.
The script holds each `--seeds` entry to the game's own seed alphabet before it writes the plan, so the file it writes is never the unreadable one.
...
The fight is `CardModel.TryManualPlay`'s request - `RequestEnqueue(new PlayCardAction)`, waited for on the engine's own `CombatHistory.CardPlaysStarted` holding the play, as the driver waits, and never on the card leaving the hand - an `EndPlayerTurnAction` enqueued as the end-turn button enqueues it, and `PotionModel.EnqueueManualUse`, under `SurvivalPlayRule`'s measured line, the survival scout's rule R3, which is the one rule the whole-act walk plays as well - its `SurvivingIndex` reads the same class, so a tuning of one is a tuning of both.
...
After the plan's count or the deadline the module writes `soak-done` under the store - the runs, their seeds and outcomes, and a null `refusal` - and calls `NGame.Quit`, the menu's own quit, so the script's release finds a client already tearing down; the script then copies only the recordings written under the profile since its launch, so the figure is the night's.
...
After a claimed reward, a purchase, a rest or an event page, Continue returns to the game's latest save - the fight won, the arrival - with those decisions discarded the same way, and a line after the continuous one says which save the game returned to and how many decisions are kept as the branch; a Continue at a shop, rest site, event or treasure room after a won fight is continuous with nothing discarded, because the restored run reads exactly as the recorder read the arrival - [demo/RECORDER-CONTINUE-RESIDUE.md](../demo/RECORDER-CONTINUE-RESIDUE.md) has those cases under the format-6 comparison, [demo/RECORDER-SAVE-POINTS.md](../demo/RECORDER-SAVE-POINTS.md) has the rollbacks driven from the game's own saves, and [demo/RECORDER-SAVE-AND-QUIT.md](../demo/RECORDER-SAVE-AND-QUIT.md) has three of them gated under format 7.
...
To exercise the reload path, continue a run from a save older than the latest one the game took - a backup of the run save from before a fight was won, put back in place of the current one: the `continuity rewound` line names the decision it came back at and the save it is behind, the overlay goes on reading RECORDING, and the rest of the run goes on being recorded into a recording the run history offers to play from and refuses to share - [demo/RUNMOBILE-REWOUND-SUBMIT.md](../demo/RUNMOBILE-REWOUND-SUBMIT.md) has that reload beside a continuous Save and Quit in the retail client, with the Mine pane's refused row and the gate's `continuity` verdict.
