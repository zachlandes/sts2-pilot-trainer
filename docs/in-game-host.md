...
An act change leaves that record untouched - a new or alternative act saves at the same call sites and adds no ancient event - so `scripts/act-topology.txt` records the acts the database ships at each index, the default and the epochs that unlock each, the unshipped `ActModel` subclasses and the members that build a run's act list, held the same way by `RunRecorderTests.TheGamesActTopologyIsTheRecordedOne`, so a build whose acts moved fails there before the fixtures' progression is stale.
...
The settle then waits for that work as it waits for any decision's, with one exception `RunRecorder.HandedToThePlayerDuring` owns and only the event option's own work opts into, through `PendingDecision.SettlesOnceHandedToThePlayer`: work that has handed the run to the player - a rewards set on offer, read where `RewardsSetSynchronizer.BeginRewardsSet` begins one, or the Crystal Sphere's screen up, read where `CrystalSphereMinigame.PlayMinigame` puts it up, either begun while this decision was the one executing - has settled, because what answers it is the player's next decisions.
Every other decision waits for its own task to finish: a set on offer beside a reward being claimed off it is not this claim's wait, and a claim, a purchase or a rest whose own work begins a set and awaits it - Orrery or the Cauldron bought from a shop, a heal under Dream Catcher or the Tiny Mailbox - runs the budget out and is refused, because the driver replays those decisions by awaiting the same task and a recording that read one as settled would replay by blocking on a set nobody answers.
