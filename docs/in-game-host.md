...
What is drawn instead is the game's own loading idiom.
`RestoringNotice.For` in `Sts2PilotTrainer.Trainer` derives the headline from the phase alone - it is the whole of what is known then - and `RestoringOverlay` in the mod instantiates the client's own `res://scenes/screens/main_menu/loading_overlay.tscn`, the scene `MegaCrit.Sts2.Core.Nodes.CommonUi.NLoadingOverlay` drives, parents it under `NGame` rather than under a run, and puts `Restoring your run` on its `%Label` with an ellipsis that animates.
It is added to the tree before the label is asked for, because the scene's own script writes the game's `Loading...` onto that label when it enters the tree.
...
Where the scene cannot be loaded, or loads without that `%Label`, the mod draws its own plate - a scrim and one centred line - rather than nothing; `RestoringSurface` in `Sts2PilotTrainer.Trainer` is the one place that decides which of the two it is, and the mod only answers whether the borrow succeeded.
