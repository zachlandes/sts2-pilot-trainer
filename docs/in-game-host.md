...
Equal in everything the game's own save can carry means the same, and is asked only once the complete digests have disagreed: a finished fight stays on the player until the next fight replaces it and `CanonicalStateProjection` keeps emitting it, so a reading taken at a shop, a rest site, an event or a loot screen after a won fight carries that fight's residue, while `SerializableRun` has no combat member and a run continued from the save carries none of it.
`ReplayTrace.SaveRepresentable` is the reading with every `combat.` field taken away unless the fight is in progress, and `RunCapture.Resume` compares the two readings that way only where one of them carries a finished fight (`ReplayTrace.CarriesFinishedCombat`): two readings that agree in what a save carries with no residue on either side while their complete digests do not differ in something else, which is a moment the journal never saw and not leftovers.
The comparison is exact where it can be: the recorder writes `ReplayTrace.SaveRepresentableDigest` beside every decision - the same filter over every projected field, the random streams and the draw order included, hashed under its own `sha256-sr:` prefix so it can never be taken for a boundary's digest - and reads the live run's at the resume, so an event page the game rolled back, which moves a stream and nothing a sample can see, is not read as nothing having happened; a line an earlier recorder wrote carries none, and there the samples are compared field for field through `ReplayTrace.SameSample`.
The complete digest stays what a boundary is identified by and what the resume asks first; nothing about `CanonicalStateProjection` changed, because every committed boundary digest is computed by it.
Before this, an honest Continue at every non-combat arrival after the first fight, and on every loot screen, resumed as a broken watch in exactly those residue fields and nothing else; `RecorderContinueTests` holds a shop arrival after a won fight to `continuous` from the saves the game itself took.
...
A journal declaring schema v3 or v2 made no promise about save points and keeps the rule it was written under, the return of a live fight to its room entry (`RunJournal.IsTheReturnOfALiveFightToItsRoomEntry`); a recording resumed from one takes no save-point line and its manifest lists none, and `RunJournal.Schema` is v4 for exactly this promise.
...
For publication, `gate` replays every discarded branch from the continued history through the decision it left from and holds the branch's state there and at its end against the engine's, compared as what a save carries where either side carries a finished fight - a branch played after a Continue was recorded on a restored run, which carries none of the last fight's residue or a fresh one in its place - and, where the save was a floor arrival, holds the branch's digest to the verified boundary there.
What this restores is continuity, and not yet publication: a run continued outside a fight after a won one validates and is the player's to play from, and `gate` still fails it on reproduction at the first non-fight arrival after the Continue, because the run the game restored carries a fresh combat state where the engine carries the finished fight's residue and `CanonicalStateProjection` emits that residue into every reading until the next fight replaces it.
[demo/RECORDER-SAVE-POINTS.md](../demo/RECORDER-SAVE-POINTS.md) has the retail run that shows it.
The projection has to stop carrying a finished fight for such a run to be shareable, and that is its own change with its own migration, because every committed boundary digest moves with it.
