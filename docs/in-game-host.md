...
The step that ends a fight while the run goes on is read once the engine has settled, through the same pump every out-of-fight decision goes through, rather than at the moment the observer closes it: the killing action finishes before the engine ends the combat, takes the fight-won save and rolls the rewards, and rolling the rewards moves a random stream, so a reading taken there names a state the run is never in again - the headless replay reads the same step once the engine has drained, and the game's own Continue restores that save into a run that has rolled its rewards, so a resume after a loot-screen quit could never have found the state the game came back in.
