...
Nothing is recorded because the reading at attach refuses before a journal exists, so the ordinary path produces no artifact at all.
The latch is the surface half only, and it is set from a prefix on purpose: being early is what closes the window, and going quiet for a setup the game then refuses costs nothing, because the next `CleanUp` clears it.
Marking is the other half and is not the same question asked twice - `GameSessionWatch.MultiplayerSessionTookEffect` runs from a postfix and marks only when `LiveRun.ReadSession` says this client really is in a multiplayer game, because ending somebody's recording is permanent and a call the game rejected did not happen.
`RunRecorder.MultiplayerSessionStarted` is then the safety net: it marks that recording non-standard through the same `source.native.integrity` a console run is marked with and stops it there, kept and unpublishable.
No v0.111.0 caller is known to reach either setup member while a run is in progress, which is why the branch is not expected to fire - that is what the sources here establish and it is not a proof of unreachability.
Its test drives `RunRecorder.MultiplayerSessionStarted` directly, and a second test drives the prefix to assert it changes no live recording, because a real multiplayer reading is not something a test process can produce.
The path marks the journal and never calls `Finish`, so it writes no manifest saying `integrity = non-standard`; that is consistent rather than a gap, because the ordinary path creates no artifact at all.
