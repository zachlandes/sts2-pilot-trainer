...
Where the scene cannot be loaded, or loads without that `%Label`, the mod draws its own plate - a scrim and one centred line - rather than nothing; the sentence is `RestoringNotice`'s either way, so the borrow decides what the notice looks like and never what it says.
