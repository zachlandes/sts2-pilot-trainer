...
A run continued outside a fight after a won one is therefore continuous, validates, is the player's to play from and passes `gate`: the retail run in [demo/RECORDER-SAVE-POINTS.md](../demo/RECORDER-SAVE-POINTS.md) failed reproduction at its first non-fight arrival after the Continue while the projection carried the finished fight, and [demo/RECORDER-SAVE-AND-QUIT.md](../demo/RECORDER-SAVE-AND-QUIT.md) is the same Save and Quit passing every condition once it did not.
