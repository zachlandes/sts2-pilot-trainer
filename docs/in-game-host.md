...
The fight is `CardModel.TryManualPlay`'s request - `RequestEnqueue(new PlayCardAction)` - an `EndPlayerTurnAction` enqueued as the end-turn button enqueues it, and `PotionModel.EnqueueManualUse`, under `SurvivalPlayRule`'s measured line, the survival scout's rule R3, which is the one rule the whole-act walk plays as well - its `SurvivingIndex` reads the same class, so a tuning of one is a tuning of both.
