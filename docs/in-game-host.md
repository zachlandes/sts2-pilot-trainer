...
`RunJournal.Schema` is v5: v4 made the promise that every save the game asked for is on the file, and v5 that no digest hashes a finished fight, so every complete digest on the file is one this projection reproduces; a journal declaring any older schema is refused on resume, as a v1 always was, because a recording continued from one would be two projections in one file, and the run is simply not continued as a recording.
