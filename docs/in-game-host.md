...
That is the safety net rather than the mechanism, and on v0.111.0 its marking branch cannot fire at all: `SetUpNewMultiplayer` and the `SetUpSavedMultiplayer` state machine both open by throwing when the run's `State` is already set, so a multiplayer session can only be set up after the previous run went through `CleanUp` - whose postfix has already cleared `RunRecorder.Active`.
Its test drives `GameSessionWatch.MultiplayerSessionSetUp` directly because the engine cannot produce that state under a live recording, which is what makes it a path to read rather than one a build exercises.
