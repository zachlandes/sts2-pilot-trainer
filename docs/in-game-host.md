...
The driver replays those decisions the same way, through `RunDriver.SettleOrHandOver`: the queue drained, and a task still open with a set on offer handed over rather than awaited on the one thread the answering decisions arrive on, looked at again at the next action for a refusal it ended in; each host reads one state, the set on offer and the engine quiet, and `GeneratedCoverageTests` holds the recording of each to a fresh replay through `TraceParity`.
