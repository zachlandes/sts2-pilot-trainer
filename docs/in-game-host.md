...
`ReplayTrace.SaveRepresentable` is the reading with every `combat.` field taken away unless the fight is in progress, `ReplayTrace.SaveRepresentableDigest` is its digest under a prefix that cannot be mistaken for a boundary's, and `RunCapture.Resume` compares the two readings that way only where one of them carries a finished fight (`ReplayTrace.CarriesFinishedCombat`): two readings whose samples agree with no residue on either side while their complete digests do not differ in something the sample does not carry, which is a moment the journal never saw and not leftovers.
...
Before this, an honest Continue at every non-combat arrival after the first fight, and on every loot screen, resumed as a broken watch in exactly those residue fields and nothing else; `RecorderContinueTests` holds a shop arrival after a won fight to `continuous` and a loot-screen Continue after a claim to a rewind rather than a hole, from the saves the game itself took.
...
The decisions past that point were played and then abandoned, and they go where the game's own rollback puts an unwound fight: `RunCapture.RollBack` cuts the history back to the decision the game came back to - the opening reading included - and keeps what it removed as a discarded branch, written to the journal with `reload` on it so `RunJournal.Parse` reads it without the room-entry rule the game's own rollback is held to.
