...
Every excusal carries a class, and the map says which classes it admits for each point: `no-producer-on-this-build`, `multiplayer-only`, `screen-without-headless-host`, `retail-only-timing`, `not-replayable`, `reached-by-the-win`, `not-choosable`, `offered-only-to-another-character` and `not-projectable` are derived from the walks and the host's own tables, `generated` is held by a merge-gate row, and `not-on-the-route` is admissible only where nothing is derived.
...
## The act-1 leftovers, 2026-09-18

The release critical path's fourth slice took what was left `not-on-the-route` in the first act and needed no survival work: 98 such points before it, 79 after, every one of the 79 naming a second act, another character, a modifier, a state the line never holds or the fight the line does not survive to.

- **The blessing rows.**
  One row per relic Neow deals that produces no seam - the sixteen the producer rows had no reason to take - each on a seed hunted so Neow offers it, the opening answered by the relic's id and nothing asked past it (`GeneratedCoverageTests.BlessingRows`); every one found its seed on the first candidate whose opening offered the relic, and the table runs in under a minute.
  With the producer rows they are held to every blessing the build offers a singleplayer run, less Winged Boots, taken by the flight regression, and Massive Scroll, which Neow offers only to a run with another player in it.
- **The seams whose producers a first act deals.**
  The plan named these seams with relics the producer rows already hold; the map on this build lists other producers under them, and the rows follow the map.
  Punch Off's nab offers a relic from inside the event (`reward-kind:relic @ EventModel.GenerateInitialOptions`), and the nab row now claims it; the Trash Heap's dive deals one of five relics by the event's own roll, and on a hunted seed it is the Dream Catcher, whose card at the next heal is `reward-kind:card @ AbstractModel.TryModifyRestSiteHealRewards` - the row rests after the event (`EventRow.ThenRests`) and the ask is the rest; the Gremlin Merc's death spawns a Fat Gremlin holding the gold the merc stole, which gives it back on its own death (`HeistPower.BeforeDeath`, `reward-kind:gold @ AbstractModel.BeforeDeath`), and a heist row was hunted for it: the merc is the Underdocks' fourth fight at the earliest, and the mechanical line reached that fight and won it on none of 25 seeds that deal the merc among the first four, ten dying on the way and fifteen finding no route to four fights past a question mark, so the seam keeps an excusal naming the power and the count for the fifth slice's survival work.
  What else stays `not-on-the-route` among the seams names producers no first act deals: the Fake Merchant's and the Battleworn Dummy's rewards are act 2's and act 3's; Royalties' gold is the Regent's card, The Hunt's card the Silent's and the Necrobinder's Grimoire the removal's; and four seams list a modifier as their only producer (Midas, Vintage, Draft), which no standard run carries and the preflight admits no other mode - a derived class for that is a follow-up.
- **The loss outside a fight.**
  The walk holds on to the Slippery Bridge past every warning (`WalkPolicy.ChooseItUntilTheRunEnds`) until the hold's damage kills the player, and the recording ends `lost` on that option with the run over at it, replayed to parity.
...
The event-rows mechanism of the coverage design put a row on the merge gate for every option of every event a question mark opens on this build: 148 rows over 46 events (`GeneratedCoverageTests.EventRows`), each on a seed hunted so the act's first question mark rolls an event and the act's shuffled set deals that one, on the acts list the event is reachable on - the default progression for the Overgrowth's twenty events and the shared pool, its Underdocks variant for the Underdocks' ten, and the Hive or Glory alone for their sixteen, on the ancient rows' footing and said so in each row's excusal.
The walk routes to the mark, answers the event's pages by key through the recorded `ChooseEventOption`, fights where an option fights and resumes the event afterwards, claims what an option offers, and replays what the recorder wrote to parity; the whole table runs in about a minute and a half.
Beside the table the thief row kills the Thieving Hopper holding the card it stole on a run of the Hive alone and claims the special card off the loot screen, so the special-card reward and both of its seams are held by rows.
The rows retire the 46 events, their 148 options, the special-card reward and seventeen seams the events' pages produce or their fights reach by co-occurrence - the removal, upgrade, transformation, enchantment and deck-grid prompts events open, the potion and custom rewards their pages offer, the Battleworn Dummy's rewards as its event resumes - and twenty locked options are derived as not choosable (`DecisionSurface.NotChoosableOptions`) rather than excused by hand.
The mechanism found four things, each fixed beside the rows and each a replay refusal a player's own recording would have met:

- **Every act after the first opens on the map.**
  `RunManager.EnterAct` enters the starting point itself only for the first act; after that the act change clears the visited coordinates and the map screen lights the starting point alone, so the player's first decision of act 2 is a `MapMove` to it from no node.
  The driver refused that move as unreachable, on every recording of a run past its first act; `MapTravelRule.TravelableFrom` now answers a run standing on no node the way the map screen does, and `ReplayRefusalRegressionTests` holds the move on the fixture seed.
  No committed recording reaches a second act and the won-run proof plays one act, which is how it stayed unnoticed.
- **Five events fault headlessly on the scene tree.**
  Dense Vegetation, Jungle Maze Adventure, Amalgamator, Punch Off and the Trial dereference a scene-tree singleton without a null-conditional, the option's task faults on the null, and the game's `TaskHelper.RunSafely` swallows the fault, so the event stood on the same page and the replay blamed the recording at its next decision.
  `PresentationStandIns` rewrites those calls at the call site in event bodies only, and `EventOptionWork` reads the option's task so the driver refuses a fault by name; `EventOptionFaultTests` holds the refusal with a stand-in fault.
- **An option's work is read when the recorder reads it.**
  Amalgamator combines the cards, sleeps three hundred milliseconds of real time and only then adds the combined card; a replay that read the decision as the action queue went idle was one card short of the recorder's reading and failed parity at the run's end.
  The driver now waits for the option's task, or for the hand-over to the player the recorder's `HandedToThePlayerDuring` names, before it reads.
- **A fight inside an event.**
  The Battleworn Dummy, Dense Vegetation, Punch Off and the Lantern Key push their fight over the event, and the retail proceed off that fight's loot screen resumes the event through the private `ResumePreviousRoom`; the press is no decision and the recorder writes nothing for it.
  `RunDriver.ResumeTheEventTheFightWasFoughtIn` makes that transition where the engine would, headlessly only.

What the rows do not reach is what only the second act on deals, and the reason is survival: the journey's mechanical line survives a first act on none of 300 hunted seeds with every room type on the route and none of 200 on the cheapest, so nothing hunted reaches a second act's question mark.
Darv is not a question mark's event at all - it is dealt to one act after the first as the run is generated (`RunManager.GenerateRooms`) and rolled as that act's opening ancient - and it, its twelve options and the four relics only it deals keep an excusal saying so, as do the nine events the game allows only from the second act on or in the second act alone, and the card-removal reward, which only a Necrobinder's Forbidden Grimoire puts on the loot screen (a character row of the fifth stage as well as a survival seed).
Three options are off the route for their own reasons - Colorful Philosophers' Ironclad on an Ironclad run, Zen Weaver's acupuncture at 250 gold, Self-Help Book's page with nothing to enchant - War Historian Repy is reached only by a second question mark after the Lantern Key, and the Grave of the Forgotten needs a power in the deck.
The one seed the line is known to survive a first act on, the whole-act fixture's 67L571H38L, opens its second act on Orobas, which the ancient rows already retire, so it serves no Darv row; Darv is dealt to one act after the first as the run starts and needs an act-1-surviving seed whose act 2 rolls Darv, which nothing has yet.
...
## The second-act rows, 2026-09-19

The sixth stage of the coverage design walked on from the second act's opening room: `GeneratedCoverageTests.SecondActAncientRows` and `SecondActEventRows` carry a whole first act into the second act under `WalkPolicy.AskInTheNextAct` and put on the merge gate what only the second act deals.

**The hunt.**
A second act's ancient is rolled as the run is generated and is the seed's own reading (`SeedHunt.Opening.NextActAncientId`); which relics it offers is rolled as its room is entered from an RNG seeded by the run's seed and the event's id, and which event the second act's first question mark opens is the first entry of the act's shuffled set, past the one the ancient's room consumes, that the run allows and has not visited by the time it gets there.
Neither is cheap to read off a fresh run, so the hunt walked: 1,000 of `SeedHunt.Candidates` in order as the Defect, whose measured line beats the first act's boss on half of them, each survivor carried through the transition to the second act's opening room and then by the cheapest route to its first question mark, reading the ancient, its offer and what the mark opened; one walk takes under half a second headlessly and the thousand under ten minutes.
Darv opens the second act on about one surviving seed in nine, and each of its eleven other options was offered on six to twenty-seven of them; each of the seven events the game allows only from the second act on or in the second act alone opened the first mark on twelve to seventeen; Pael offered Pael's Growth on twenty-five and Tezcatara Pumpkin Candle on thirty-five.
The Necrobinder's line survives a first act on one seed in eighteen, and Darv offered Dusty Tome on three of those; on every one the tome's ancient card was Forbidden Grimoire, and the walk claimed the removal its power earned on the second act's fights on two.
A row's seed is pinned with its criterion beside it as every row's is; here the walk is the criterion for the offer and the mark, and the walk refuses by name a second act that opens elsewhere or offers other relics (`SyntheticFixtureGenerator.OpenTheActAndMeetTheAsk`) and a run of one act before a step, because past a lone act's boss is the victory room and a Darv row satisfied there would be about a room Darv is never rolled for.

**The rows.**
Thirteen second-act ancient rows: Darv's eleven, each on a seed whose second act opens on Darv offering the relic, obtained through the recorded `ChooseEventOption` naming both; Dusty Tome's as the Necrobinder, walking the second act's fights until one offers the removal Forbidden Grimoire's power put on its loot screen and claiming it (`RewardKindToClaim`), which retires `card_removal` and its seam; and Pael's Growth's and Pumpkin Candle's, the two rest options act 2's ancients' relics add that the ancient rows fell short of, taken at the second act's first rest site with a whole first act's deck behind them.
Twenty second-act event rows: every option of Crystal Sphere, Doll Room, Potion Courier, Ranwid the Elder, Stone of All Time, Symbiote and Welcome to Wongos, Zen Weaver's acupuncture at 250 gold with the first act's purse, and Colorful Philosophers' Ironclad as the Defect, which retires the one option `offered-only-to-another-character` had classed; the Crystal Sphere's cells are revealed through the headless stand-in, and its screen's verb and seam keep `screen-without-headless-host` for the retail half.
The Fake Merchant beside them: it constructs no option and stands finished from its first page, drawing a shop of its own, so the walk empties its relic shelf through the same purchase a merchant room's takes and the recording replays to parity.
Nothing met on the first act counts as a row's ask (`MeetTheAskIf` reads `_beforeTheAskedAct`), because every decision there is the fixture's own.
The Byrdonis Nest's take row now rests next and hatches the egg (`EventRow.ThenRests`), retiring `HATCH` and its seam.
The rows run in about three minutes.

**A ninth derived class.**
The Fake Merchant is projected from no decision: the format reads an event off the option chosen in it (`DecisionFacts.Of`), its purchases are projected as the shop kind and its thrown potion as the verb, and no checkpoint names the event, so neither the event nor the seams only it produces - its shelf, and the rug and relics its fight's loot offers, which the map lists at no hook of the event's own - is ever reached by a recording or by the co-occurrence read.
`not-projectable` is derived off the same walk that lists every other event's keys (`DecisionSurface.OptionlessEvents`: an event an act reaches whose code constructs no option and builds none at runtime) and admitted for that event and for a seam every producer of which is such an event; the three points are excused in it with the row that buys from the merchant named, and a build that gives the event an option takes it off the list by itself.

**What is left, and why.**
`not-on-the-route` went from 73 points to 25 over `manifests/`.
What remains is what only a third act deals - the Grave of the Forgotten, Glory's own event; War Historian Repy, which the Lantern Key card puts in the next question mark only where the act index is 2 (`LanternKey.ModifyNextEvent`), so no run of the Hive alone and no walk that stops in the second act reaches it, and the earlier excusal's "second question mark after the Lantern Key" was wrong; Meat Cleaver's cook, Tanx's - beside the Relic Trader, which wants five tradable relics where the walk reaches the second act's mark holding four to six with the starter among them and opened it on none of the thousand, the Self-Help Book's empty page, which wants a deck with nothing left to enchant, and the seams whose producers are a modifier no standard run carries, a character's own card (Royalties' gold, The Hunt's card) or a card-prompt hook a first act's fights never open.
Those are owed to a walk into a third act, on a line measured past its second, or to the retail soak, and each is named in `DecisionExcusals` with what stands in the way.
...
Darv kept its excusal with that reason, as did its twelve relics, the card-removal reward whose ancient card only its Dusty Tome deals, and the special-card reward the Lantern Key or a thief's death deals; this section called each a question-mark hunt like an event's, and the event rows found Darv is no question mark's event but the opening ancient of one act after the first, which is corrected there.
Three items the third stage was to retire were deferred from this change to the event-rows change, ratified 2026-09-17: the card-removal reward, the special-card reward and the events only the Hive and Glory roll.
The event rows of 2026-09-18 retire the second and third; the first waits on a second act, as that section says.
