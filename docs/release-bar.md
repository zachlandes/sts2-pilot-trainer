...
The seventeen events only the Hive and Glory roll are not in this change either: reaching one is the question-mark walk - a route into a `?`, an event's pages answered, a seed hunted so the map rolls that event - which every event row of the fourth stage needs as well, and the rows for both go in on that mechanism.
...
- **A purchase the engine makes for itself.**
  ...
  `ReplayRefusalRegressionTests` holds the walk into the shop to a recording with no purchase in it and to parity.
