...
Its one-resource pack supplies the 64×64 `mod_image.png` shared by the game's mod list and Runmobile's Compendium entry; `has_pck: true`, `dependencies: []`, and `affects_gameplay: false` remain the manifest's complete declaration.
...
The retail client walks the complete installed `Runmobile` tree recursively and opens every `*.json` as a mod manifest.
Only the root `Runmobile.json` may carry a top-level `id`, `name`, `author`, `description`, or `version`; another such file either registers as a second mod or logs an error when its `id` is absent.
That is why the prepared release info is written byte-for-byte as `release_info.json.copy`, which `Sts2PilotTrainer.Bootstrap` owns and `ModHostBoundaryTests` holds the complete installed artifact to.
...
The field is what the release bar reads a recorder's age off: `RecordingStanding` classes a recording whose `recorder_version` is below the running one, or is that unstamped `1.0.0.0`, as an older recorder's, so a release is also the bump that separates the candidate's recordings from what the recorder before it wrote; `docs/release-bar.md` owns the rule.
The field is therefore held at build time to the grammar `RecorderVersion` reads - `major.minor.patch`, an optional `-prerelease` ordered below its release, never build metadata after a `+` - and a declaration outside it is a build error naming the field rather than an abort at the first recording of a corpus; `VersionAgreementTests` drives that target with both kinds.
