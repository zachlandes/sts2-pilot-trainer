...
What gives is the two grids, and each gives the same way: it pages, down to one row, behind the run-history screen's own arrows in the first and last place of the page, exactly as the strip pages its floors, so every relic and every card is a page away at full size and nothing is scrolled, clipped or shrunk.
