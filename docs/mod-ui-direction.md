...
**The lock is the stats screen's image, hung off the tab's corner**: `submenu_lock.png`, which `stats_screen.tscn` centres across its dimmed Achievements label, sits instead as a small badge - half the run-history entry's marker - off the Community tab's top-right corner, above the band its word is drawn in, the way the run-history entry hangs its quest badge off an icon's corner.
...
The word's box is left whole: narrowing it for a lock beside the word made the game's own auto-size draw "Community" smaller than "Mine", because `MegaLabel` sizes each word to its own box and no inset makes two words agree.
...
`LibraryPaneArt.Lay` is the one place the pane's parts are placed against each other: the identity over the relic rows, the deck count on the first row's line, the strip under them, the deck's tiles under the strip, the facts and the verdict under those, and the plate's ribbons at the pane's foot, side by side at the panel's own ribbon size, the first at the pane's left edge and the last at its right, with `PlateAir` under them so the last is not flush against the panel's own primary ribbon below.
Those deck tiles are what exposed an upgraded card drawing its name rather than its portrait, so `ModelArt.ArtId` strips the `+N` upgrade suffix before the model database is asked; that fix rode along with the pane work because the tiles were the first surface here to draw an upgraded card.
...
The strip never gives: its marker is the run-history entry's own size, and that size is the point of it.
What gives is the two grids, and each gives the same way: it pages, down to one row, behind the run-history screen's own arrows in the first and last place of the page, so every relic and every card is a page away at full size and nothing is scrolled, clipped or shrunk.
The strip pages the same way, and its arrows stand in the column the game's own settings paginator gives its arrows - read off `paginator.tscn` through `NativePaginatorArt.ArrowWidth`, narrower than a marker's column - with the marker columns between them, so a paged strip keeps the game's marker and on this build's pane shows four floors a page.
The deck takes its rows first, up to every row it has, and the relics page into the remainder, on the opened run and on the browser pane alike - `RunBrowser` hands the browser pane the run's deck as well as its count, and the retail Mine capture in `demo/RUNMOBILE-NATIVE-TYPE.md` shows the tiles there - because the deck is what a pane exists to show; a recording that carries no deck gives the relics the room.
Every pane is held to its height, the opened run's included, because at the holder's own size a mid-act run's relics and an ordinary deck ran past the panel's ribbon.
A pane that cannot give one row of each beside the strip refuses by name rather than drawing one part over another; before any of this the plate was placed from the bottom and drawn upward over whatever was there, which put "Open the run" across the floor numerals and the version line under "Share this run".
...
Why a plate ribbon is refused - a rewound or kept-only run's Submit - is that ribbon's own hover, through `ScreenRow.Tooltip`, never a third fact line, and a refused ribbon anywhere on this surface is the same `Refuse`: dimmed, out of the focus column, and saying why on hover where it has no line under it.
...
The deck count shares the first row's line, right-aligned and drawn once whichever page is up, and that row holds only as many places as leave the count's measured width clear, because a row filled edge to edge put the count over its last icons; a count too wide to share a row with even one relic takes a line of its own over the rows.
