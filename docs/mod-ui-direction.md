...
What is drawn instead is not the tag at all but the game's own loading idiom: `RestoringNotice` in `Sts2PilotTrainer.Trainer` derives the headline from the phase, and `RestoringOverlay` in the mod instantiates the client's own `res://scenes/screens/main_menu/loading_overlay.tscn` - the scene `MegaCrit.Sts2.Core.Nodes.CommonUi.NLoadingOverlay` drives, confirmed against the package index for v0.111.0 - parents it under `NGame` rather than a run, and puts the mod's own headline on its `%Label` with an ellipsis that animates.
The borrow is preferred and never required, and the sentence is `RestoringNotice`'s whichever drawing is up.
The borrowed scene ships hidden - its root is saved with `visible = false`, and the game's own callers hold it and set `Visible` when they show it - so the mod switches it on the same way after adding it, and treats a root that is not on screen once asked to be as a borrow that failed.
Borrowing it without that put a present-but-invisible overlay over the whole wait and never fell through to the plate: measured in the retail client, which drew nothing and logged nothing.
Where the scene cannot be loaded, or loads without the `%Label` the label lookup covers, the mod draws its own plate instead - a scrim and one centred line at the native heading role - because a build that has moved the scene must cost the native look and never the notice: a minute of blank screen is the defect this surface exists to remove.
...
Borrowing the scene rather than styling one is the same rule as every other surface here - the sizes the game draws its own furniture at are the only ones that read as native.
