...
`scripts/game-build.txt` is the one build this checkout supports: version, build date, commit, branch, the sha256 of the pristine `sts2.dll` and `main_assembly_hash`, the hash the game's own `release_info.json` names by that key (not the model-id-database content hash the manifests record), every field read off the installation it was taken from by `./scripts/bootstrap.sh`, which writes the same record beside the prepared set as `build/lib/game-build.txt`.
