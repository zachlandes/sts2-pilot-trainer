...
*2026-09-19T19:14:06Z by Showboat 0.6.1*
<!-- showboat-id: 283d37aa-fb0d-4c64-a94a-f3fad431522d -->
...
**The claim being tested.** A night is one command. It holds the machine awake for its own lifetime, installs the mod, writes the soak profile's `settings.json` whole with the night's plan, launches the retail client through `./scripts/retail-client.sh` with `--headless`, waits for the mod to write `soak-done` under the profile's store, releases through the same helper, copies the profile's recordings into dated evidence, runs parity and coverage over that copy beside `manifests/`, prints the third release figure, and exits non-zero on a parity failure or a run the mod could not finish cleanly. Nothing under the store is deleted. The stand-in plays the soak's client: under `--headless` it writes `soak-done` the way `RetailSoak` writes it and quits the way `NGame.Quit` does, and without the flag it never writes it, which is what makes the flag load-bearing. `RetailSoakScriptTests` holds all of it; this is the same script, watched.

Two things are stood in for and one is real. The executable is `tests/stand-ins/retail-client.sh`, and the process table is a `ps` that lists the stand-in's pid as the game, because the real table names a script by its interpreter. The recording is real: `native-67L571H38L-20260919-091953`, a run the mod itself played and recorded under `--headless` on v0.111.0 during the soak probe of 2026-09-19 - two fights, a loot screen, a give-up, thirty decisions - copied out of the store on this machine into the sandbox's, because a stand-in plays no run.

## The sandbox: a home, a stand-in game, a profile pointer, and one real recording

```bash
rm -rf build/soak-demo && mkdir -p build/soak-demo && cd build/soak-demo && HOME_DIR="$PWD/home" && USER_DIR="$HOME_DIR/Library/Application Support/SlayTheSpire2" && GAME="$PWD/install/SlayTheSpire2.app/Contents/MacOS/Slay the Spire 2" && mkdir -p "$(dirname "$GAME")" tools "$USER_DIR/default/2/modded" "$USER_DIR/Runmobile/default/2/modded/profile1/recordings" && cp ../../tests/stand-ins/retail-client.sh "$GAME" && chmod +x "$GAME" && printf "{\n  \"last_profile_id\": 1,\n  \"schema_version\": 2\n}\n" > "$USER_DIR/default/2/modded/profile.save" && cp "$HOME/Library/Application Support/SlayTheSpire2/Runmobile/default/1/modded/profile3/recordings/native-67L571H38L-20260919-091953."* "$USER_DIR/Runmobile/default/2/modded/profile1/recordings/" && cat > tools/ps <<'EOF'
#!/usr/bin/env bash
...
home/Library/Application Support/SlayTheSpire2/Runmobile/default/2/modded/profile1/recordings/native-67L571H38L-20260919-091953.journal.jsonl
home/Library/Application Support/SlayTheSpire2/Runmobile/default/2/modded/profile1/recordings/native-67L571H38L-20260919-091953.replay.json
install/SlayTheSpire2.app/Contents/MacOS/Slay the Spire 2
tools/ps
```

`env.sh` is what the tests set on the script's process: a sandbox `HOME`, where the stand-in writes what it saw, where the stand-in's pid is listed, and where the stand-in writes `soak-done` - the path the mod writes it to, under the profile's store. The real recording sits in the sandbox store's `recordings/` exactly where the mod puts one.
...
```output
19:14:06Z retail soak: 1 run(s) of CHARACTER.IRONCLAD at ascension 0, seeds [67L571H38L], deadline 1m
19:14:06Z store        : <worktree>/build/soak-demo/home/Library/Application Support/SlayTheSpire2/Runmobile/default/2/modded/profile1
19:14:06Z evidence     : build/soak-demo/night-1
19:14:06Z wrote <worktree>/build/soak-demo/home/Library/Application Support/SlayTheSpire2/Runmobile/default/2/modded/profile1/settings.json
19:14:06Z launching: ./scripts/retail-client.sh launch --owner retail-soak 2026-09-19 --client-id 2 --headless --game <worktree>/build/soak-demo/install/SlayTheSpire2.app/Contents/MacOS/Slay the Spire 2
client       : pid 12590
...
working dir  : <worktree>/build/soak-demo/home/Library/Application Support/sts2-pilot-trainer/retail-client/cwd.JPb6sb
...
19:14:17Z soak-done arrived after 10s
19:14:17Z soak-done: { "schema": "sts2-pilot-trainer/retail-soak-done/v1", "finished_at_utc": "2026-09-19T19:14:08Z", "runs_planned": 1, "runs_started": 1, "runs": [ { "run": 1, "seed": "STANDIN", "outcome": "ended" } ], "recorder_version": "stand-in"}
19:14:17Z copied 1 recording(s) to build/soak-demo/night-1/recordings
19:14:17Z parity:
...
19:14:28Z the third figure: parity holds over the night's copy beside manifests/; coverage holds; 0 run(s) to read; evidence in build/soak-demo/night-1
exit 0
```

The plan was written whole, the launch went through the helper with `--headless` last, `soak-done` was written two seconds after the launch and found on the script's first ten-second poll, the release found the client already gone, the one recording was copied and never moved, and both numbers hold: the night's copy is one native recording at parity, beside the committed corpus's two older-recorder natives and its one reconstruction. What the stand-in saw is the proof of the launch arguments:
...
```output
cwd	<worktree>/build/soak-demo/home/Library/Application Support/sts2-pilot-trainer/retail-client/cwd.JPb6sb
entries	
...
---
{
  ...
  "finished_at_utc": "2026-09-19T19:14:08Z",
  "runs_planned": 1,
  "runs_started": 1,
  "runs": [
...
The same night over a copy of the recording whose journal was edited at its fifth decision - the player's health read as 1 before it - which is the shape a recorder timing defect takes in a morning's `parity.txt`.

```bash
( source build/soak-demo/env.sh && J="$HOME/Library/Application Support/SlayTheSpire2/Runmobile/default/2/modded/profile1/recordings/native-67L571H38L-20260919-091953.journal.jsonl" && cp "$J" build/soak-demo/journal.good && python3 - "$J" <<'EOF'
import json, sys
path = sys.argv[1]
...
```output
19:14:28Z retail soak: 1 run(s) of CHARACTER.IRONCLAD at ascension 0, seeds [fresh each], deadline 1m
19:14:28Z store        : <worktree>/build/soak-demo/home/Library/Application Support/SlayTheSpire2/Runmobile/default/2/modded/profile1
19:14:28Z evidence     : build/soak-demo/night-2
19:14:28Z wrote <worktree>/build/soak-demo/home/Library/Application Support/SlayTheSpire2/Runmobile/default/2/modded/profile1/settings.json
19:14:28Z launching: ./scripts/retail-client.sh launch --owner retail-soak 2026-09-19 --client-id 2 --headless --game <worktree>/build/soak-demo/install/SlayTheSpire2.app/Contents/MacOS/Slay the Spire 2
client       : pid 34457
executable   : <worktree>/build/soak-demo/install/SlayTheSpire2.app/Contents/MacOS/Slay the Spire 2
arguments    : --force-steam=off --clientId=2 --headless
save tree    : user://default/2
working dir  : <worktree>/build/soak-demo/home/Library/Application Support/sts2-pilot-trainer/retail-client/cwd.iVSAAy
log          : <worktree>/build/soak-demo/home/Library/Application Support/sts2-pilot-trainer/retail-client/client.log
record       : <worktree>/build/soak-demo/home/Library/Application Support/sts2-pilot-trainer/retail-client/owner
owner        : retail-soak 2026-09-19
release with : ./scripts/retail-client.sh release
19:14:39Z soak-done arrived after 10s
19:14:39Z soak-done: { "schema": "sts2-pilot-trainer/retail-soak-done/v1", "finished_at_utc": "2026-09-19T19:14:30Z", "runs_planned": 1, "runs_started": 1, "runs": [ { "run": 1, "seed": "STANDIN", "outcome": "ended" } ], "recorder_version": "stand-in"}
19:14:39Z copied 1 recording(s) to build/soak-demo/night-2/recordings
19:14:39Z parity:
...
19:14:50Z the third figure: parity does not hold over the night's copy beside manifests/; coverage holds; 0 run(s) to read; evidence in build/soak-demo/night-2
exit 1
```
...
```bash
( source build/soak-demo/env.sh && cp build/soak-demo/journal.good "$HOME/Library/Application Support/SlayTheSpire2/Runmobile/default/2/modded/profile1/recordings/native-67L571H38L-20260919-091953.journal.jsonl" && export FAKE_SOAK_OUTCOME=unknown-state && ./scripts/retail-soak.sh --game "$FAKE_GAME_EXECUTABLE" --skip-install --client-id 2 --runs 1 --stop-after-minutes 1 --out build/soak-demo/night-3 ); echo "exit $?"
```

```output
19:14:50Z retail soak: 1 run(s) of CHARACTER.IRONCLAD at ascension 0, seeds [fresh each], deadline 1m
19:14:50Z store        : <worktree>/build/soak-demo/home/Library/Application Support/SlayTheSpire2/Runmobile/default/2/modded/profile1
19:14:50Z evidence     : build/soak-demo/night-3
19:14:50Z wrote <worktree>/build/soak-demo/home/Library/Application Support/SlayTheSpire2/Runmobile/default/2/modded/profile1/settings.json
19:14:50Z launching: ./scripts/retail-client.sh launch --owner retail-soak 2026-09-19 --client-id 2 --headless --game <worktree>/build/soak-demo/install/SlayTheSpire2.app/Contents/MacOS/Slay the Spire 2
client       : pid 58333
executable   : <worktree>/build/soak-demo/install/SlayTheSpire2.app/Contents/MacOS/Slay the Spire 2
arguments    : --force-steam=off --clientId=2 --headless
save tree    : user://default/2
working dir  : <worktree>/build/soak-demo/home/Library/Application Support/sts2-pilot-trainer/retail-client/cwd.aFkuVY
log          : <worktree>/build/soak-demo/home/Library/Application Support/sts2-pilot-trainer/retail-client/client.log
record       : <worktree>/build/soak-demo/home/Library/Application Support/sts2-pilot-trainer/retail-client/owner
owner        : retail-soak 2026-09-19
release with : ./scripts/retail-client.sh release
19:15:01Z soak-done arrived after 10s
19:15:01Z soak-done: { "schema": "sts2-pilot-trainer/retail-soak-done/v1", "finished_at_utc": "2026-09-19T19:14:52Z", "runs_planned": 1, "runs_started": 1, "runs": [ { "run": 1, "seed": "STANDIN", "outcome": "unknown-state" } ], "recorder_version": "stand-in"}
19:15:01Z copied 1 recording(s) to build/soak-demo/night-3/recordings
19:15:01Z a run ended 'unknown-state'; read godot.log
19:15:01Z parity:
  PARITY      native-67L571H38L-20260919-091953  30 in the journal, 30 replayed
  older recorder native-3LACFJ5NJ371-20260906-015901
              journal written by recorder 'runmobile-recorder/1.0.0.0'; this build's recorder is 'runmobile-recorder/0.2.0', and what changed between them is why the journal is not held to a replay. The manifest still replays on this build, so what it reached is credited to coverage
...
19:15:12Z the third figure: parity holds over the night's copy beside manifests/; coverage holds; 1 run(s) to read; evidence in build/soak-demo/night-3
exit 4
```
...
```output
19:15:12Z retail soak: 1 run(s) of CHARACTER.IRONCLAD at ascension 0, seeds [fresh each], deadline 1m
19:15:12Z store        : <worktree>/build/soak-demo/home/Library/Application Support/SlayTheSpire2/Runmobile/default/2/modded/profile1
19:15:12Z evidence     : build/soak-demo/night-4
19:15:12Z wrote <worktree>/build/soak-demo/home/Library/Application Support/SlayTheSpire2/Runmobile/default/2/modded/profile1/settings.json
19:15:12Z launching: ./scripts/retail-client.sh launch --owner retail-soak 2026-09-19 --client-id 2 --headless --game <worktree>/build/soak-demo/install/SlayTheSpire2.app/Contents/MacOS/Slay the Spire 2
client       : pid 79608
executable   : <worktree>/build/soak-demo/install/SlayTheSpire2.app/Contents/MacOS/Slay the Spire 2
arguments    : --force-steam=off --clientId=2 --headless
save tree    : user://default/2
working dir  : <worktree>/build/soak-demo/home/Library/Application Support/sts2-pilot-trainer/retail-client/cwd.brrCt4
log          : <worktree>/build/soak-demo/home/Library/Application Support/sts2-pilot-trainer/retail-client/client.log
record       : <worktree>/build/soak-demo/home/Library/Application Support/sts2-pilot-trainer/retail-client/owner
owner        : retail-soak 2026-09-19
release with : ./scripts/retail-client.sh release
19:15:25Z no soak-done after 10s; releasing the client
19:15:25Z the night ended without soak-done; nothing is measured. Read <worktree>/build/soak-demo/home/Library/Application Support/SlayTheSpire2/logs/godot.log.
exit 3
```

## The store is never deleted from, and a profile that is not a soak's is refused

Everything the four nights left in the sandbox store: the recording, both files, the settings the script wrote and the last `soak-done`. Then the refusal that protects a player's profile: a settings file with no `retail_soak` plan in it is somebody's, and the script will not replace it whole unless told to.

```bash
( source build/soak-demo/env.sh && S="$HOME/Library/Application Support/SlayTheSpire2/Runmobile/default/2/modded/profile1" && find "$S" -type f | sed "s|$S/||" | sort && printf "{\"schema\":\"sts2-pilot-trainer/runmobile-settings/v1\",\"sharing_service_url\":\"https://example.test/\"}\n" > "$S/settings.json" && ./scripts/retail-soak.sh --game "$FAKE_GAME_EXECUTABLE" --skip-install --client-id 2 --runs 1 --stop-after-minutes 1 --out build/soak-demo/night-5; echo "exit $?"; cat "$S/settings.json" )
```
...
```bash
./scripts/retail-soak.sh --help 2>&1 | head -30
```
