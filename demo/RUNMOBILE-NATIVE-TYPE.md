...
The playback tag copies the top bar's health, timer and deck-counter roles, the native popup's row-label role, and the game's native hover-tip roles; the fight result panel copies the native post-fight popup's heading, body and button roles plus the top-bar deck counter for figures.
