...
public static class RecorderCopy
{
    ...
    /// <summary>The same row once the run is over and the recording is whole: every
    /// decision from the run's start to its end, whether won or lost, is on disk and the
    /// run is the player's to play from.</summary>
    public const string RecordingComplete = "RECORDING COMPLETE";
