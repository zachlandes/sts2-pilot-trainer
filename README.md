...
A run you used the game's own console in is still recorded and still kept on your machine; the recording says which it is, and one that says so is never publishable and never played from, because what a command did to the run is not among the decisions the history holds.
Every place that would offer either says so before you press anything.
...
[docs/release-bar.md](docs/release-bar.md) is the release procedure both numbers make, and the first measurement.
...
recordings against - reports every way a decision can reach the game on this build
as claimed by a row of that table or excused in writing (`--update` rewrites
`scripts/decision-ledger.txt`, the ledger on this build), and checks that the
