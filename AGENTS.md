...
./scripts/choice-entry-points.sh            # which game types reach each card or relic prompt is still what we recorded (--update to re-record)
```
...
They need the prepared game, so the local merge gate runs them and a hosted runner never does.
