...
An event's option is a point of its own (`event-option`, projected off `option_key` under the event's id and under `EVENT.NEOW` for a blessing): an ancient's options are the game's own `AllPossibleOptions` keyed the way `RunDriver.OptionKey` keys one, every other event's are read off its IL, and an event that builds a key at runtime is derived in `DecisionSurface.RuntimeBuiltOptionKeys` or refused by name.
...
Every excusal carries an `ExcusalClass` beside its prose, and `DecisionSurface.AdmissibleExcusals` says which the map admits per point: five are derived - no producer on this build (nothing produces the seam and no engine path constructs the thing), multiplayer only (constructed under the game's own player-count branch), a screen the headless host has none for (`ScreenStandIns.StoodInFor`), retail-only timing (`RunDriver.RetailOnlyWindow`), not replayable (an alternative whose after-action keeps the selection open, by `ManifestCardSelector.EndsTheSelection`) - `generated` is admissible everywhere because a merge-gate row outranks any reason, and `not-on-the-route` only where nothing is derived; `coverage` fails an excusal in a class the map does not admit the way it fails a stale one.
...
What no generated walk reaches is excused in `DecisionExcusals` by what its producer is: the three screens the headless host has none for, the undo of an ended turn (which needs the window the retail client leaves open before the enemy turn), the reroll the driver refuses, the mend a singleplayer run is never offered, and every point whose producer the fixture seed's route does not pass - a range prompt, the card-removal and special-card rewards, the sacrifice and the rest options a relic or the Byrdonis Egg adds (a relic granted outside a recorded decision is state no replay of the recording reproduces), every event, since the route refuses question marks and an event is reached only where the map rolls it, and every ancient, since the act's roll is what reaches one.
