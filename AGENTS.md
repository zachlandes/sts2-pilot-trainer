...
Every way of leaving a run and coming back to it, or giving it up, is the save-Continue-give-up matrix in that same class: one `[GameTheory]` of 23 rows, each driven through the game's own saves and restore, held to the recorder's contract at every Continue and to one of four verdicts once the run is given up, with a refused Continue failing on the fields the restored run differed in.
