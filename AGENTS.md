...
Every excusal carries an `ExcusalClass` beside its prose, and `DecisionSurface.AdmissibleExcusals` says which the map admits per point: nine are derived - no producer on this build (nothing produces the seam and no engine path constructs the thing), multiplayer only (constructed under the game's own player-count branch, or a verb the client offers only while another player has not acted, `RunDriver.OfferedOnlyWithAnotherPlayer`), a screen the headless host has none for (`ScreenStandIns.StoodInFor`, for a verb and for a seam every answer of which is such a verb), retail-only timing (derived for nothing on this build; the undo it was thought to class is offered to another player), not replayable (no recording of it can replay on this build: an alternative whose after-action keeps the selection open, by `ManifestCardSelector.EndsTheSelection`, or an event option keyed by a LocString's raw text, which the recorder writes as a localized title the driver never matches, by `DecisionSurface.TitleKeyedConstructions`), reached by the win (the Architect, its options and the seams it alone produces), not choosable (an event option constructed locked, with no work, where the player cannot afford the real one, which its own button refuses to press, by `DecisionSurface.NotChoosableOptions`), offered only to another character (an event option constructed under the event's own guard on the owner's card pool, Colorful Philosophers' five, by `DecisionSurface.OptionsWithheldFromTheCharacter`, which names the character each is withheld from), not projectable (an event that constructs no option and the seams only it produces, the Fake Merchant's, by `DecisionSurface.OptionlessEvents`: its decisions are purchases and a thrown potion, which the format projects as the shop kind and the verb, and an event is projected from the option chosen in it, so no recording projects it and no co-occurrence read names it) - `generated` is admissible everywhere because a merge-gate row outranks any reason, and `not-on-the-route` only where nothing is derived; `coverage` fails an excusal in a class the map does not admit the way it fails a stale one, and an excusal whose prose names who produces the point carries that producer's id beside it (`Excusal.Producers`), held by `coverage` to the producers the map lists for the point (`DecisionCoverage.ProducersListedAt`) and printed as a misnamed producer where the map lists it nowhere there, because a build that moves a producer leaves the class true and the sentence false.
...
**The second-act rows walk on from the second act's opening room, and are the same class again on the default progression.**
`GeneratedCoverageTests.SecondActAncientRows` and `SecondActEventRows` each carry a whole first act into the second act under `WalkPolicy.AskInTheNextAct`, as the Defect, whose measured line survives a first act on half the hunted seeds, or as the Necrobinder where the ask is its own: the eleven options of Darv the character row does not take, each on a seed hunted so the second act opens on Darv offering the relic (`SeedHunt.Opening.NextActAncientId` is the seed's reading and the offer is rolled as the room is entered, so the walk refuses by name a second act that opens elsewhere or offers other relics, and a run of one act by name before a step); the card-removal reward, through Dusty Tome's Forbidden Grimoire on a Necrobinder run, claimed off the second act's fights (`RewardKindToClaim`); Pael's Growth's clone and Pumpkin Candle's kindle at the second act's first rest site; every option of the seven events the game allows only from the second act on or in the second act alone, Zen Weaver's acupuncture with a whole first act's purse and Colorful Philosophers' Ironclad as the Defect, each at the second act's first question mark on a seed hunted so it opens the event; and the Fake Merchant, which constructs no option and draws a shop of its own, emptied through the same purchase a merchant room's takes.
Nothing met on the first act counts as the ask, because every decision there is the fixture's own.
The Byrdonis Nest's take rests next and hatches the egg (`EventRow.ThenRests`).
What no generated walk reaches is excused in `DecisionExcusals` by what its producer is: the two screens the headless host has none for and no walk answers (the bundle screen is stood in for and the Scroll Boxes row answers it through the stand-in), the undo of an ended turn and the mend, which the client offers only to a run with another player in it, the reroll the driver refuses, the Architect and its options and seams, which the won-run proof reaches, and what only a third act deals: the Grave of the Forgotten, Glory's own, War Historian Repy, which the Lantern Key puts in the third act's question mark and no earlier one (`LanternKey.ModifyNextEvent`), and Meat Cleaver's cook, Tanx's; the Relic Trader wants five tradable relics no walk into a second act holds, and Self-Help Book's empty page a deck no walk builds.
Those are owed to a walk into a third act, on a line measured past its second, or to the retail soak.
...
**What CI cannot run is recorded by name.** On a runner without the game, the 187
tests named in `scripts/expected-hosted-skips.txt` skip out of the 296 cases
...
datum where it runs, so a run with the game reports more cases than 296.
...
`RetailSoakModule` draws nothing and is off unless an isolated profile's `settings.json` carries a `retail_soak` plan, which only `scripts/retail-soak.sh` writes: it arms at the main menu, adopts the game once it has a plan, starts standard runs through `NGame.StartNewSingleplayerRun` with the recorder attached, plays them through what a click issues - `TryManualPlay`, an enqueued `EndPlayerTurnAction`, `EnqueueManualUse`, under `SurvivalPlayRule`'s measured line - and the game's own screen and room handlers, replaces the game's cheating combat and event handlers with its own state-driven loops, paces every decision on `RunRecorder.Settled`, gives a run up through the pause menu at any state it does not know, writes `soak-done` and calls the game's own quit, and a plan it refuses before starting a run is written to `soak-done` as zero runs with the sentence and quits the same way, so the script reads the refusal in the morning rather than waiting out the deadline; `RetailSoakGuardTests` refuses by IL every call to `PowerCmd`, `CreatureCmd.Kill`, `CardSelectCmd.UseSelector`, `CardCmd.AutoPlay`, the seed override, an FTUE, epoch or preference write.
