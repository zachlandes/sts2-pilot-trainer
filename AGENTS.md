...
**`EngineCommands.Verify()` asks a third question, the other way round: every way a decision can reach the game on this build is claimed by a row or excused in writing.**
...
A candidate accounted for neither way is `UNCLASSIFIED`, which is what a game update's new message or screen is before anybody looks at it, and `engine-commands` reports DRIFTED on it; an excusal naming a candidate no walk produces is stale and refused the same way.
