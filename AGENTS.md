...
What no generated walk reaches is excused in `DecisionExcusals` by what its producer is: the three screens the headless host has none for, the undo of an ended turn and the mend, which the client offers only to a run with another player in it, the reroll the driver refuses, the Architect and its options and seams, which the won-run proof reaches, and every point whose producer the fixture seed's route does not pass - a range prompt, the card-removal and special-card rewards, the sacrifice and the rest options a relic or the Byrdonis Egg adds (a relic granted outside a recorded decision is state no replay of the recording reproduces), every event, since the route refuses question marks and an event is reached only where the map rolls it, and every ancient, since the act's roll is what reaches one.
...
**What CI cannot run is recorded by name.** On a runner without the game, the 176
tests named in `scripts/expected-hosted-skips.txt` skip out of the 262 cases
...
datum where it runs, so a run with the game reports more cases than 262.
...
Every other decision is read once the engine's own work for it is finished, with one exception the event option's work alone opts into: a run handed to the player inside that work - a rewards set on offer, the Crystal Sphere's screen up - which `RunRecorder.HandedToThePlayerDuring` owns, asked of that decision and never of the run; a claim, a purchase or a rest whose own work begins a set is waited for and refused when its task does not finish, because the driver replays it by awaiting the same task; an event option's work is a task the synchronizer keeps to itself, read on its way past through `EventOption.Chosen`, because a decision settled on the action queue alone was read during the hit animation Brain Leech's RIP awaits before it rolls its reward.
