...
./scripts/act-topology.sh                   # which acts ship at each index, which is the default, what unlocks each and who builds a run's act list are still what we recorded (--update to re-record)
```
...
That record cannot see an act change, because a new or alternative act keeps every `SaveManager.SaveRun` call site and every ancient-event subclass where they were, so `ActTopology` in `Sts2PilotTrainer.Engine` is the reading that changes instead: every act `ModelDb.Acts` ships at each index, whether it is the index's default, the epochs its own `IsUnlocked` names, its room and floor counts, every `ActModel` subclass the database leaves out, and every member that calls `ActModel.GetDefaultList` or `GetRandomList`.
`RunRecorderTests.TheGamesActTopologyIsTheRecordedOne` holds it to `scripts/act-topology.txt`, regenerated with `./scripts/act-topology.sh --update` in the same change, and `TheFixturesProgressionIsTheGamesDefaultOne` holds `RecordedActWalk.Acts`, the progression every headless fixture starts its run on, to the default act at each index, so a build that adds an act, moves one or changes the default fails there before a fixture drifts.
