...
That recording is `continuity = rewound`: whole, so the validator takes it and the run is the player's to play from, and never theirs to share, which the run-history plate's submit row, `RunSharing`'s seal and the gate's `continuity` condition all refuse - the validator cannot, because it is what `RecordedFightEntry` asks before it stands anybody in a fight.
