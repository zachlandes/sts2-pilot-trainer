...
An excusal's "no committed recording reaches it" is a placeholder the generated-sequence and retail-soak slices retire, and is not an acceptable excuse at release for a point a player can reach on this build.
...
A sender or syncer type with a body the IL reader could not read whole is a candidate the walks may never see, so `DecisionSurface.UnreadableLedgerBodies` names each with its body count, `DecisionLedger.UnreadableExcused` carries the written reason each is still whole - read off the decompiled build - and `VerifyLedger` names one excused neither way or excused for nothing; the record carries that set and the loader's unloadable types beside the candidates.
