...
./scripts/save-points.sh                    # who calls SaveManager.SaveRun and which types subclass AncientEventModel are still what we recorded (--update to re-record)
```
...
Which members reach `SaveManager.SaveRun`, and which types this build subclasses `AncientEventModel` with, are read from the assembly rather than assumed: `RunRecorderTests.TheGamesSaveContractIsTheRecordedOne` holds both to `scripts/save-points.txt`, regenerated with `./scripts/save-points.sh --update` in the change that adopts a build, so a game update that moves where the run saves or changes which ancient events exist shows up as a diff rather than as a save/resume test that keeps passing against a set the game no longer has.
