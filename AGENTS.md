...
The strip's floor markers are the run-history screen's own icons through `FloorMarkerArt`, at the history entry's own size, and `LibraryPaneArt.CellGeometry` is the one place a cell's icon, numeral, badge, ring and bookmark tab are placed so they cannot collide; `LibraryPaneArt.Lay` is the same one place for the pane - the plate keeps the foot, the strip gives first and the relic rows then scroll at the run-history holder's own size read through `RelicHolderArt`, and a pane short of even that refuses rather than overlapping.
