...
Every other decision is read once the engine's own work for it is finished, or has handed the run to the player - a rewards set on offer, the Crystal Sphere's screen up - which `RunRecorder.HandedToThePlayerDuring` owns, asked of the decision whose work began the wait and never of the run; an event option's work is a task the synchronizer keeps to itself, read on its way past through `EventOption.Chosen`, because a decision settled on the action queue alone was read during the hit animation Brain Leech's RIP awaits before it rolls its reward.
