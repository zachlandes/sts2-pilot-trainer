...
Retention is asked for before any module surface can draw, at both of the menus a module's surface hangs off: the singleplayer-menu patch asks unconditionally, and the main-menu row asks through `RunmobileMod.EnsureAdopted` before it adds anything.
The singleplayer patch attempts adoption only where a module needs it - so a build on which every module declines never reaches adoption from there and still honours a purge, as does one the engine layer refuses to adopt, in every profile the player switches to.
