...
That scan reads the game against the vendored Godot stubs, so a body whose signature or call site names a member the stubs have not got cannot be read at all; a fourth fact holds the set of types with such bodies to `scripts/unreadable-choice-scan-bodies.txt`, regenerated by the same `--update`, so a stub gap fails as a diff naming the type rather than thinning the enumeration or hiding a body that opens a screen.
