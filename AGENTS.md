...
Every control it contributes names the mod, through `LibraryCopy.OurSetting`: they stand in the game's own General list in the game's own type, so nothing else says which of them Runmobile put there, and the main-menu control is the one exception because it already names the mod inside its own sentence.
