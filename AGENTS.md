...
What no generated walk reaches is excused in `DecisionExcusals` by what its producer is: the three screens the headless host has none for, the undo of an ended turn (which needs the window the retail client leaves open before the enemy turn), the reroll the driver refuses, the mend a singleplayer run is never offered, and every point whose producer the fixture seed's route does not pass - a range prompt, the card-removal and special-card rewards, the sacrifice and the relic-added rest options (a relic granted outside a recorded decision is state no replay of the recording reproduces), and every event, since the route refuses question marks and an event is reached only where the map rolls it.
