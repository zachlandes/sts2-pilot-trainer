...
Placing it compares the complete digest first and then, only where one reading carries a finished fight, the part of each reading the game's own save can carry (`ReplayTrace.SaveRepresentable`), because a finished fight stays on the player until the next one and no save carries it; [docs/in-game-host.md](docs/in-game-host.md) owns the rule and its limit, and `CanonicalStateProjection` is not where it lives.
