...
Moving the policy removes nothing where it stands and forgets that profile's retention latch through `RecordingRetention.ReapplyPolicyAtNextMenu`, so the next main menu applies what the row promised rather than the next launch; a store that cannot yet be asked - no save profile chosen - is a fact on the facts and one refused line, never an exception out of `MyRunsSettings.Build`.
