...
The strip's floor markers are the run-history screen's own icons through `FloorMarkerArt`, and `LibraryPaneArt.CellGeometry` is the one place a cell's icon, numeral, badge and ring are placed so they cannot collide.
...
One heading over that row names the mod, through `LibraryCopy.OurSettingsHeading`: the controls stand in the game's own General list in the game's own type, so nothing else says which of them Runmobile put there, and a tag in front of every caption grew three of the four controls wider than the row they are laid out in.
