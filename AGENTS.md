...
The ancient rows found a fifth: the purchases Lord's Parasol makes for itself as the merchant is entered, through the purchase member with `ignoreCost` set, were recorded as the player's, ahead of the move that opened the shop, and refused at replay in the room the move left; a purchase the engine makes for itself is not a decision and `RunRecorder.ShopPurchased` records none.
