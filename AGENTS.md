...
That scan reads the game against the vendored Godot stubs, so a body whose signature or call site names a member the stubs have not got cannot be read, and a type whose shape needs one cannot be loaded; a fourth fact holds both sets - types with unreadable bodies, and the loader's names for the types it could not load - to `scripts/unreadable-choice-scan-bodies.txt`, regenerated by the same `--update`, so a stub gap fails as a diff naming the type rather than thinning the enumeration or hiding a body that opens a screen.
