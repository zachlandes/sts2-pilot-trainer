...
Placing it compares the complete digest first and then, only where one reading carries a finished fight, the part of each reading the game's own save can carry - exactly, through the `sr_digest` the recorder writes beside every decision (`ReplayTrace.SaveRepresentableDigest`), or field for field through `ReplayTrace.SaveRepresentable` where a line an earlier recorder wrote carries none - because a finished fight stays on the player until the next one and no save carries it; [docs/in-game-host.md](docs/in-game-host.md) owns the rule and its limit, and `CanonicalStateProjection` is not where it lives.
...
The step that ends a fight the run survives is read once the engine has settled, through the pump, because the game rolls the rewards after the killing action finishes and a reading taken before that names a state no replay holds; the retail client rolls them later still, on its own clock, so a resume matches that one entry through the next decision's `before_sr_digest` and never through its own.
That rollback stays continuous, keeps the undone decisions as discarded evidence in the journal and manifest - `source.native.save_points` beside them, captured, present and empty where the game saved only at run start - and resumes the replayable history at the save's decision; the gate replays every branch from that decision and holds its state either end, as what a save carries where either side carries a finished fight.
...
Continuity is what that restores; a run continued outside a fight after a won one still fails `gate` on reproduction at its next non-fight arrival, because the restored run carries a fresh combat state where the engine carries the finished fight's residue and `CanonicalStateProjection` emits it, and that projection change is the migration reserved above rather than part of this rule.
A journal declaring an older schema made no promise about save points and keeps its own rule, a live fight's return to its room entry; the reload is the `rewound` case below, and no other mismatch is repaired.
...
Every decision in a journal carries the reading it began from as well as the one it settled into, because the comparison instant is before each decision; `RunJournal.Schema` is v4 - v2 added the before-reading, v3 the bookmark line, v4 the save-point line and the promise that every save the game asked for is on the file - a v3 or v2 journal is read as-is because every line of each is a v4 line, and a v1 journal is refused on resume rather than repaired.
